...
- **Scope + leaves:** `@aoagents/ao` → `@slievr/athene` (umbrella); `@aoagents/ao-cli` →
  `@slievr/cli`; `@aoagents/ao-core` → `@slievr/core`; `@aoagents/ao-web` → `@slievr/web`;
  `@aoagents/ao-notifier-macos` → `@slievr/notifier-macos`; `@aoagents/ao-integration-tests`
  → `@slievr/integration-tests`; `@aoagents/ao-website` → `@slievr/website`;
  `@aoagents/ao-plugin-*` → `@slievr/plugin-*` (25 plugins).
- **Binary:** `ao` → `athene`.
...
1. `@aoagents/ao-plugin-` → `@slievr/plugin-`
2. `@aoagents/ao-cli` → `@slievr/cli`
3. `@aoagents/ao-core` → `@slievr/core`
4. `@aoagents/ao-web` → `@slievr/web`
5. `@aoagents/ao-notifier-macos` → `@slievr/notifier-macos`
6. `@aoagents/ao-integration-tests` → `@slievr/integration-tests`
7. `@aoagents/ao-website` → `@slievr/website`
8. `@aoagents/ao` → `@slievr/athene`  (umbrella; safe only after the above)
...
(incl. subpath exports like `@slievr/core/types`, `/utils`, `/scm-webhook-utils`), vitest
...
- `packages/core/src/plugin-registry.ts` — `BUILTIN_PLUGINS` (25 entries `pkg: "@aoagents/ao-plugin-*"` → `"@slievr/plugin-*"`). Caught by step 1 (full strings), but verify.
...
- `packages/core/src/update-cache.ts` — `getInstalledAoVersion()` candidates `@aoagents/ao/package.json`, `@aoagents/ao-cli`, `@aoagents/ao-web` → `@slievr/athene`, `@slievr/cli`, `@slievr/web`. Keep cache path `~/.cache/ao/` (compat).
- `packages/ao/bin/postinstall.js` — `findPackageUp(__dirname, "@aoagents", "ao-web"|"ao-cli")` and better-sqlite3/`@slievr/core` lookups → update scope + leaf.
- `scripts/rebuild-node-pty.js` — `@aoagents/ao-web`, `@aoagents/ao-cli` lookups → `@slievr/web`, `@slievr/cli`.

### 3. External-plugin name regex
- `packages/core/src/config.ts:427` regex `^ao-plugin-(?:runtime|agent|workspace|tracker|scm|notifier|terminal)-(.+)$` derives a plugin's short name from its package name. Built-ins are unaffected (hardcoded list), but to support future `@slievr/plugin-*` plugins, broaden to `^(?:ao-)?plugin-(?:runtime|agent|workspace|tracker|scm|notifier|terminal)-(.+)$` (accept both old and new prefixes). Update the adjacent comments/examples.

### 4. Binary rename `ao` → `athene`
- `bin` field in `packages/ao/package.json` and `packages/cli/package.json`: `"ao"` → `"athene"`.
- Rename shim `packages/ao/bin/ao.js` → `bin/athene.js` (content: `import "@slievr/cli"`); update the `bin` path.
...
- Update scope filters in `.github/workflows/*.yml` (`ci.yml`, `release.yml`, `canary.yml`):
  `@aoagents/ao-web` → `@slievr/web`; version-source path `packages/ao/package.json` stays
  (dir unchanged) but the package name it reads is now `@slievr/athene`.
...
- `pnpm install` (regenerates `pnpm-lock.yaml` with new names), `pnpm -r build`,
  `pnpm typecheck`, `pnpm -r --filter '!@slievr/web' test`.
...
2. `pnpm install && pnpm -r build && pnpm typecheck && pnpm -r --filter '!@slievr/web' test` all pass.
...
5. Publish dry run: `pnpm -r build && pnpm changeset publish --dry-run` (or a CI run on a
   throwaway changeset) shows `@slievr/*` packages targeted, none `@aoagents`.
6. After first real publish: `npm i -g @slievr/athene` on a clean machine → `athene` works.
