...
Automated weekly release notes for `slievr/Athene`. The cron job pulls `main`, runs `run.py`, and posts the output to Discord. No manual redeployment — PRs merged into `main` take effect on the next run.
...
Requirements: `gh` CLI authenticated against `slievr/Athene`, `python3` ≥ 3.9. No other dependencies — the runner only uses the stdlib and shells out to `gh`.
...
| `--repo` | `slievr/Athene` | Target repo. |
...
1. **Title + date.** `# Athene — Week of {Mon DD, YYYY}`. Use the Monday of the report week, not the run day.
