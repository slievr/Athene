...
describe("activity-events DB unavailable warning", () => {
  ...
  const originalDebug = process.env["ATHENE_DEBUG"];

  beforeEach(() => {
    __resetActivityEventsDbWarningForTests();
    vi.spyOn(console, "warn").mockImplementation(() => {});
    delete process.env["ATHENE_DEBUG"];
  ...
  afterEach(() => {
    ...
    if (originalDebug === undefined) {
      ...
      process.env["ATHENE_DEBUG"] = originalDebug;
    }
    vi.restoreAllMocks();
  });
  ...
  it("suppresses non-events invocations unless ATHENE_DEBUG=1", () => {
    const err = new Error(
      "Could not locate the bindings file. Tried:\n → /tmp/better_sqlite3.node",
    );
