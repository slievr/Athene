...
beforeEach(() => {
  ...
  // ATHENE_SHELL short-circuits auto-detection; clear unless a test sets it.
  delete process.env["ATHENE_SHELL"];
});
...
describe("resolveWindowsShell", () => {
  ...
  it("honors ATHENE_SHELL override before any auto-detection", async () => {
    setPlatform("win32");
    const saved = process.env["ATHENE_SHELL"];
    process.env["ATHENE_SHELL"] = "C:\\custom\\pwsh.exe";
    ...
    try {
      ...
      if (saved !== undefined) process.env["ATHENE_SHELL"] = saved;
      else delete process.env["ATHENE_SHELL"];
    }
  });
  ...
  ])("ATHENE_SHELL=%s infers args from basename", async (override, expectedArgs) => {
    setPlatform("win32");
    const saved = process.env["ATHENE_SHELL"];
    process.env["ATHENE_SHELL"] = override;
