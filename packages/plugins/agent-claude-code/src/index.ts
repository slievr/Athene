 ...
 * `process.env.AO_CALLER_TYPE === "orchestrator"`, so worker sessions are
 * unaffected and `setupWorkspaceHooks` stays role-agnostic.
 *
...
// Runtime gating: only act in orchestrator sessions. Workers and every other
// caller type are unaffected even though the hook is installed everywhere.
