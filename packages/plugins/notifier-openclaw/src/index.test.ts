...
describe("notifier-openclaw", () => {
  ...
  it("formats v3 CI notifications as a compact OpenClaw brief", async () => {
    ...
    expect(body.message).toContain(
      "[#1579 - Normalize AO notifier payloads](https://github.com/slievr/Athene/pull/1579)",
    );
    expect(body.message).toContain("**Checks**");
    expect(body.message).toContain(
      "- [typecheck](https://github.com/slievr/Athene/pull/1579/checks): `failed/FAILURE`",
    );
  ...
  it("formats v3 merge notifications with status and links", async () => {
    ...
    expect(body.message).toContain(
      "- [Pull request](https://github.com/slievr/Athene/pull/1579)",
    );
  });
