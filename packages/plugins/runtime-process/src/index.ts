...
export function create(): Runtime {
  ...
  return {
    ...
    async create(config: RuntimeCreateConfig): Promise<RuntimeHandle> {
      ...
      if (isWindows()) {
        ...
        try {
          ...
          // Sideband registration so `athene stop` can find and graceful-kill this
    ...
    async destroy(handle: RuntimeHandle): Promise<void> {
      ...
      if (!entry) {
        // Process was spawned by a different Node.js process (e.g. athene spawn).
 ...
 *
 * Invoked by `athene stop` on Windows so orphaned pty-hosts (whose session JSON
...
export async function sweepWindowsPtyHosts(): Promise<{
  ...
  const { getWindowsPtyHosts, unregisterWindowsPtyHost } = await import("@made-by-moonlight/athene-core");
