...
describe("spawn command", () => {
  ...
  it("rejects more than one positional arg with replacement usage", async () => {
    ...
    expect(errors).toContain("athene spawn [issue]");
  ...
  it("groups cross-project issues and routes each to the correct project", async () => {
    ...
    expect(spawnCalls).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ projectId: "agent-orchestrator", issueId: "10" }),
        expect.objectContaining({ projectId: "x402-identity", issueId: "20" }),
      ]),
    );
