...
describe("update-check", () => {
  ...
  describe("classifyInstallPath", () => {
    ...
    it("returns 'git' when repo root has .git", () => {
      ...
      mockReadFileSync.mockReturnValue(JSON.stringify({ name: "@made-by-moonlight/athene" }));
  ...
  describe("getUpdateCommand", () => {
    it("returns 'athene update' for git installs", () => {
      expect(getUpdateCommand("git")).toBe("athene update");
    });

    it("returns npm install command for npm-global installs", () => {
      expect(getUpdateCommand("npm-global")).toBe("npm install -g @made-by-moonlight/athene@latest");
    });

    it("returns pnpm add command for pnpm-global installs", () => {
      expect(getUpdateCommand("pnpm-global")).toBe("pnpm add -g @made-by-moonlight/athene@latest");
    });

    it("returns npm install command for unknown installs", () => {
      expect(getUpdateCommand("unknown")).toBe("npm install -g @made-by-moonlight/athene@latest");
    });
  });
  ...
  describe("fetchLatestVersion", () => {
    it("returns latest dist-tag from registry by default (stable channel)", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        "https://registry.npmjs.org/@made-by-moonlight%2Fathene",
    ...
    it("returns null when dist-tags missing", async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => ({ name: "@made-by-moonlight/athene" }),
      });
      expect(await fetchLatestVersion("stable")).toBeNull();
    });
  ...
  describe("checkForUpdate", () => {
    ...
    it("uses npm registry and npm-global command for npm-global installs", async () => {
      ...
      expect(info.latestVersion).toBe("0.3.0");
      expect(info.recommendedCommand).toBe("npm install -g @made-by-moonlight/athene@latest");
    });
    ...
    it("uses npm registry and pnpm-global command for pnpm-global installs", async () => {
      ...
      expect(info.latestVersion).toBe("0.3.0");
      expect(info.recommendedCommand).toBe("pnpm add -g @made-by-moonlight/athene@latest");
    });
    ...
    it("uses cached git state without consulting npm registry", async () => {
      ...
      mockReadFileSync.mockImplementation((path: string) => {
        ...
        return JSON.stringify({ name: "@made-by-moonlight/athene" });
      });
    ...
    it("checks git source installs against origin/main when cache is stale", async () => {
      ...
      expect(info.isOutdated).toBe(true);
      expect(info.latestVersion).toBe("origin/main");
      expect(info.recommendedCommand).toBe("athene update");
      expect(mockFetch).not.toHaveBeenCalled();
      const written = JSON.parse(mockWriteFileSync.mock.calls[0][1] as string);
  ...
  describe("maybeShowUpdateNotice", () => {
    ...
    beforeEach(() => {
      ...
      origNotifier = process.env["ATHENE_NO_UPDATE_NOTIFIER"];
      origArgv = process.argv;
      Object.defineProperty(process.stderr, "isTTY", { value: true, configurable: true });
      delete process.env["CI"];
      delete process.env["AGENT_ORCHESTRATOR_CI"];
      delete process.env["ATHENE_NO_UPDATE_NOTIFIER"];
    ...
    afterEach(() => {
      ...
      if (origNotifier !== undefined) process.env["ATHENE_NO_UPDATE_NOTIFIER"] = origNotifier;
      else delete process.env["ATHENE_NO_UPDATE_NOTIFIER"];
    ...
    it("prints update notice when cache shows outdated version", () => {
      ...
      expect(output).toContain("npm install -g @made-by-moonlight/athene@latest");
    });
    ...
    it("does not print placeholder 0.0.0 when the current version is unknown", () => {
      ...
      expect(output).toContain("npm install -g @made-by-moonlight/athene@nightly");
    ...
    it("prints git update notice from cached git state", () => {
      ...
      expect(output).toContain("Run: athene update");
    ...
    it("does not print when ATHENE_NO_UPDATE_NOTIFIER=1", () => {
      process.env["ATHENE_NO_UPDATE_NOTIFIER"] = "1";
      maybeShowUpdateNotice();
      expect(stderrSpy).not.toHaveBeenCalled();
    });
  ...
  describe("getUpdateCommand — channel-aware (Section B)", () => {
    it("uses @nightly tag for nightly channel", () => {
      expect(getUpdateCommand("npm-global", "nightly")).toBe("npm install -g @made-by-moonlight/athene@nightly");
      expect(getUpdateCommand("pnpm-global", "nightly")).toBe("pnpm add -g @made-by-moonlight/athene@nightly");
      expect(getUpdateCommand("bun-global", "nightly")).toBe("bun add -g @made-by-moonlight/athene@nightly");
    });

    it("uses @latest tag for stable + manual channels", () => {
      expect(getUpdateCommand("npm-global", "stable")).toBe("npm install -g @made-by-moonlight/athene@latest");
      expect(getUpdateCommand("npm-global", "manual")).toBe("npm install -g @made-by-moonlight/athene@latest");
    });
