...
import { ENV, getEnvString, isEnvFlagEnabled, isWindows, withLegacyEnvAliases } from "@made-by-moonlight/athene-core";
import {
  classifyInstallPath,
...
export function resolveRepoRoot(): string {
  const override = getEnvString(ENV.REPO_ROOT);
  if (!override) {
  ...
  if (!isValidRepoRootForLayout(resolved, DEFAULT_SCRIPT_LAYOUT)) {
    ...
    throw new Error(`${ENV.REPO_ROOT}=${override} does not look like ${expected}`);
  }
...
export function resolveScriptLayout(): ScriptLayout {
  const override = getEnvString(ENV.SCRIPT_LAYOUT);
  if (
    isEnvFlagEnabled(ENV.DEV) &&
...
export async function runRepoScript(scriptName: string, args: string[]): Promise<number> {
  ...
  let shellOverride = getEnvString(ENV.BASH_PATH);
  ...
  // fall back to ATHENE_BASH_PATH, throw with guidance if neither is found.
    ...
    if (!detected) {
      throw new Error(
        "Cannot run repo scripts on Windows without bash. " +
          ...
          `set ${ENV.BASH_PATH} to a bash executable ` +
...
function spawnAndWait(
  ...
  return new Promise<number>((resolveExit, reject) => {
    const child = spawn(shell, shellArgs, {
      cwd: repoRoot,
      env: withLegacyEnvAliases({
        ...process.env,
        [ENV.REPO_ROOT]: repoRoot,
        [ENV.SCRIPT_LAYOUT]: scriptLayout,
      }),
      stdio: "inherit",
      windowsHide: true,
    });
