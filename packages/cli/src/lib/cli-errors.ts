...
export function formatCommandError(err: unknown, options: CommandErrorOptions): Error {
  ...
  if (code === "ENOENT") {
    return new Error(
      `${options.cmd} is not installed or not on PATH, so Athene could not ${action}.${formatHints(options.installHints)}`,
    );
  }

  if (code === "EACCES") {
    return new Error(
      `${options.cmd} exists but Athene could not execute it due to a permission error while trying to ${action}. ` +
