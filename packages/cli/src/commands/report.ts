 ...
 *   2. the `ATHENE_SESSION_ID` environment variable set by every agent plugin.
 *
...
function resolveSessionId(explicit: string | undefined): string {
  ...
  const fromEnv = getEnvString(ENV.SESSION_ID)?.trim();
  if (fromEnv) return fromEnv;
  console.error(
    chalk.red(
      `No session provided. Pass a session name or set ${ENV.SESSION_ID} (set automatically inside managed sessions).`,
    ),
  );
  process.exit(1);
}
...
export function registerAcknowledge(program: Command): void {
  program
    ...
    .argument("[session]", `Session ID (defaults to ${ENV.SESSION_ID})`)
...
export function registerReport(program: Command): void {
  ...
  program
    ...
    .option("-s, --session <id>", `Session ID (defaults to ${ENV.SESSION_ID})`)
