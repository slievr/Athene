...
<Callout type="info" title="What AO installs">
  The published package is `@slievr/athene`. It provides the global `ao` command and includes the built-in CLI, dashboard, and plugin packages.
</Callout>
...
<Tabs items={["npm", "pnpm", "yarn", "from source"]}>
  ...
  <Tab value="pnpm">
  ```bash
  pnpm add -g @slievr/athene
  ```
  </Tab>
  <Tab value="yarn">
  ```bash
  yarn global add @slievr/athene
  ```
  </Tab>
  ...
  pnpm install
  pnpm build
  pnpm --filter @slievr/athene link --global
  ```
  </Tab>
</Tabs>
