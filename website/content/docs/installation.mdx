...
This page gets your machine ready to run Athene. By the end, the `ao` command should be on your `PATH`, your source-control CLI should be authenticated, and `athene doctor` should be able to explain what is ready or missing.

<Callout type="info" title="What AO installs">
  The published package is `@made-by-moonlight/athene`. It provides the global `ao` command and includes the built-in CLI, dashboard, and plugin packages.
</Callout>
...
<Tabs items={["npm", "pnpm", "yarn", "from source"]}>
  ...
  <Tab value="pnpm">
  ```bash
  pnpm add -g @made-by-moonlight/athene
  ```
  </Tab>
  <Tab value="yarn">
  ```bash
  yarn global add @made-by-moonlight/athene
  ```
  </Tab>
  ...
  pnpm install
  pnpm build
  pnpm --filter @made-by-moonlight/athene link --global
  ```
  </Tab>
</Tabs>
...
Run `athene start` from a repository you want AO to manage:
...
`athene doctor` checks the launcher, config, plugin resolution, runtime tools, source-control authentication, notifier setup, and stale temporary files. If it reports a fixable issue, run:
...
<Accordions>
  <Accordion title="`ao` is not found after install">
    Your global package bin directory is not on `PATH`. Check your package manager's global bin path, then reopen your shell and run `athene --version` again.
  </Accordion>
  ...
  <Accordion title="Port 3000 is already in use">
    `athene start` scans for a free port and prints the final dashboard URL. Use the URL in the command output.
  </Accordion>
  <Accordion title="The dashboard fails after an update">
    Run `athene dashboard --rebuild` to clean stale dashboard build artifacts and start again.
  </Accordion>
</Accordions>
