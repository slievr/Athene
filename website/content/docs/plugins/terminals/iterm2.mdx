...
The macOS attach experience. When you run `athene open`, it opens each session in a new iTerm2 tab (or window, with `--new-window`).
...
- **"open-iterm-tab: command not found".** Reinstall AO (`npm install -g @made-by-moonlight/athene`) — the helper binary lives in the ao package.
