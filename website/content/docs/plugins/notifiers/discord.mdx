...
```bash
athene setup discord --status
athene setup discord --refresh
```
...
| `username`     | optional | `Athene` | Override webhook default username per message                       |
...
```bash
athene notify test --to discord --template basic
athene notify test --to discord --template merge-ready --actions
```
