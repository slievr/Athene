...
```typescript
import type { PluginModule, Notifier } from "@made-by-moonlight/core";
...
Replace the placeholder `create()` body with a real implementation. Import the slot interface from `@made-by-moonlight/core`:

```typescript
import type { PluginModule, Notifier } from "@made-by-moonlight/core";
import { validateUrl } from "@made-by-moonlight/core";
```
...
All interfaces are defined in `packages/core/src/types.ts` and exported from `@made-by-moonlight/core`.
...
All utilities are exported from `@made-by-moonlight/core`. The source lives in `packages/core/src/index.ts`.
