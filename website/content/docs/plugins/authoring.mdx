...
```typescript
import type { PluginModule, Notifier } from "@made-by-moonlight/athene-core";
...
## Create a plugin with `athene plugin create`

The fastest way to start is `athene plugin create`. It runs an interactive prompt (using `@clack/prompts`) and writes a ready-to-build package.
...
Replace the placeholder `create()` body with a real implementation. Import the slot interface from `@made-by-moonlight/athene-core`:

```typescript
import type { PluginModule, Notifier } from "@made-by-moonlight/athene-core";
import { validateUrl } from "@made-by-moonlight/athene-core";
```
...
All interfaces are defined in `packages/core/src/types.ts` and exported from `@made-by-moonlight/athene-core`.
...
Use `athene plugin install ao-plugin-notifier-pagerduty` to add this block automatically.
...
## `athene plugin` subcommands

| Command | Description |
|---------|-------------|
| `athene plugin list` | List plugins from the bundled marketplace catalog. Add `--installed` to show only what's in your config. Filter by slot with `--type <slot>`. Add `--refresh` to pull the latest catalog from the registry. |
| `athene plugin search <query>` | Search the bundled catalog by name, package, description, or slot. |
| `athene plugin create [dir]` | Scaffold a new plugin package interactively. |
| `athene plugin install <reference>` | Install a plugin by marketplace ID, package name, or local path. Writes the entry to `agent-orchestrator.yaml`. |
| `athene plugin update [reference]` | Update an installer-managed plugin. Pass `--all` to update everything. |
| `athene plugin uninstall <reference>` | Remove a plugin from the config. |

---

## Core utilities available to plugins

All utilities are exported from `@made-by-moonlight/athene-core`. The source lives in `packages/core/src/index.ts`.
