...
- **By path**: `athene start ~/code/new-repo` — AO infers a project ID from the directory name and appends it.
- **By URL**: `athene start https://github.com/owner/repo` — AO clones it under `~/.agent-orchestrator/` and registers it.

Both update `agent-orchestrator.yaml` in place. Diff it if you're curious.

## Spawning in the right project

`athene spawn` auto-detects the project when you're in a worktree belonging to it. Otherwise name it:

```bash
athene spawn 42 -p api
athene batch-spawn 10 11 12 -p web
```
...
```bash
athene status                   # all projects
athene status -p api            # one project
athene status --json | jq       # scripted
athene status --watch           # live-updating table
```
...
```bash
athene stop api          # stop one project's orchestrator + dashboard
athene stop --all        # stop everything
```
