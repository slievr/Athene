...
description: Common AO problems and concrete fixes. Most start with `athene doctor`.
---

import { Accordions, Accordion } from "fumadocs-ui/components/accordion";

## First step: `athene doctor`

```bash
athene doctor
athene doctor --fix           # auto-repair safe things
athene doctor --test-notify   # send a test through each notifier
```
...
<Accordions>
  ...
  <Accordion title="Port 3000 already in use">
    AO auto-picks the next free port. You'll see the actual URL in the `athene start` output.
  </Accordion>
  <Accordion title="Dashboard crashes with 'vendor-chunks not found'">
    Stale Next.js artifacts. Run `athene dashboard --rebuild`.
  </Accordion>
</Accordions>
...
<Accordions>
  <Accordion title="`athene spawn` hangs at 'waiting for idle'">
    The agent never reached idle state. Run `athene status --watch` to see what's happening — if it's legitimately working (e.g. large install step), wait. Otherwise `athene session kill <session>` and spawn again.
  </Accordion>
  <Accordion title="Dashboard activity stuck on 'active' or 'idle'">
    `getActivityState` reads the agent's JSONL / activity log. If that file is missing or stale, you'll see wrong states. `athene doctor` flags this. See the agent's individual plugin page under [Plugins › Agents](/docs/plugins/agents) for recovery.
  </Accordion>
  <Accordion title="Agent opened a PR but the dashboard doesn't know">
    The PATH wrapper didn't record it (possible if the agent called `gh` by absolute path). Use `athene session claim-pr <pr> <sessionId>` to link retroactively.
  </Accordion>
  <Accordion title="Session won't restore">
    `athene session restore <sessionId>` relaunches the agent in the same worktree. If restore keeps failing, check the agent plugin's resume docs — some agents don't support resume at all (Cursor, Aider).
  </Accordion>
  <Accordion title="`Session can't be found` with OpenCode">
    Run `athene session remap <sessionId>` to re-discover the OpenCode mapping. Use `--force` if a stale mapping is stuck.
  </Accordion>
</Accordions>
...
<Accordions>
  <Accordion title="`athene open` prints a URL instead of opening a tab">
  ...
  <Accordion title="Child process exits immediately">
    Shell quoting issue. Pass long prompts via `--prompt` with simple ASCII, or via `athene send --file <path>`.
  </Accordion>
</Accordions>
...
- Open an issue: [`slievr/Athene`](https://github.com/slievr/Athene/issues).
When opening an issue, include:

- Output of `athene --version`
