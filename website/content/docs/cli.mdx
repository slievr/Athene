...
├── init                        (deprecated — use athene start)
...
<Callout type="warn">
  The old two-arg form `athene spawn &lt;project&gt; &lt;issue&gt;` is rejected with an error. Use `-p` or run from inside a worktree.
</Callout>

### `athene batch-spawn <issues...>`
...
If the build looks stale you'll see a suggestion to run `athene dashboard --rebuild`.
...
Runs `athene-doctor.sh` for shell-level checks, then TypeScript checks (version freshness, plugin resolution, notifier connectivity). OpenClaw is the only notifier probed without side effects.
...
### `athene session claim-pr <pr> [session]`
...
### `athene session restore <session>`
...
| Command | Purpose |
|---|---|
| `athene setup dashboard` | Configure dashboard notification retention and routing |
| `athene setup desktop` | Configure native desktop notifications |
| `athene setup webhook` | Configure a generic HTTP webhook |
| `athene setup slack` | Configure a Slack incoming webhook |
| `athene setup discord` | Configure a Discord incoming webhook |
| `athene setup composio` | Open the interactive Composio setup hub |
| `athene setup composio-slack` | Configure Slack through Composio |
| `athene setup composio-discord` | Configure Discord webhook mode through Composio |
| `athene setup composio-discord-bot` | Configure Discord bot mode through Composio |
| `athene setup composio-mail` | Configure Gmail through Composio |
| `athene setup openclaw` | Configure OpenClaw gateway notifications |
...
### `athene plugin search <query>`

> Search the bundled marketplace catalog

No flags.

### `athene plugin create [directory]`
...
### `athene plugin uninstall <reference>`
...
| `AO_PROJECT_ID` | Set by AO inside spawned sessions. Used by `athene spawn` to resolve the active project. |
