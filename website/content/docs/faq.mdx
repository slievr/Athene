...
<Accordions>
  ...
  <Accordion title="Can I write my own plugin?">
    Yes. `athene plugin create --slot <slot> --name <name>` scaffolds a starter. Plugins are tiny Node packages exporting a manifest + `create()` function.
  </Accordion>
  ...
  <Accordion title="How do I completely uninstall?">
    `npm uninstall -g @made-by-moonlight/athene`, then `rm -rf ~/.agent-orchestrator` if you want to wipe all session history.
  </Accordion>
</Accordions>
