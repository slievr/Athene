---
title: Architecture
description: How Athene fits together — plugin slots, session lifecycle, event bus, prompt assembly, and activity detection.
---

Athene (AO) is a Node.js orchestrator that spawns and manages parallel AI coding agents across isolated git worktrees. Every moving part is a plugin; the core provides the state machine, event bus, and prompt assembly that ties them together.
...
- **Role rules** — read-only investigations only; never own a PR; never use `tmux send-keys` directly; always use `athene send` / `athene spawn` to delegate.
- **Project info** — name, repo, default branch, session prefix, local path, dashboard port.
- **Quick-start commands** — `athene status`, `athene spawn`, `athene batch-spawn`, `athene session claim-pr`, `athene send`, `athene open`.
...
agent-orchestrator.yaml ──► Config Loader (Zod) ──► Plugin Registry
                                          ...
                                          │
                                          ▼
                                    Session Manager ◄─── athene spawn / athene session
                                          │
                                          ▼
