...
- Use `@made-by-moonlight/athene-core` for cross-package imports.
...
- Core utilities exported from `@made-by-moonlight/athene-core`
...
**8. Shell safety.** Any command construction must use `shellEscape()` from `@made-by-moonlight/athene-core` for all dynamic arguments. Flag raw string interpolation in shell commands.
